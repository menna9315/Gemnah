(function($) {
    ...
    $window.on("load", function() {
        var $popup = $("#newslettermodal"),
            $popupHide = "hideNewsletterModal",
            $checkboxHideBtn = $(".newsletter-modal .checkbox-hide .checkbox-hide-btn");
        // check localStorage before showing
        if(!localStorage.getItem($popupHide)) {
            setTimeout(function() {
                $popup.delay(500).modal("show");
            }, 500); // only setTimeout needed
        }
        // on checkbox click, store preference
        $checkboxHideBtn.on("change", function() {
            if($(this).is(":checked")) {
                localStorage.setItem($popupHide, "true");
            } else {
                localStorage.removeItem($popupHide);
            }
        });
        // optional: Close modal also stores preference if checked
        $popup.on("hide.bs.modal", function() {
            if($checkboxHideBtn.is(":checked")) {
                localStorage.setItem($popupHide, "true");
            }
        });
        // newsletter-product js
        var swiper = new Swiper(".swiper#newsletter-product-slider", {
            loop: true,
            rewind: true,
            slidesPerView: 3,
            spaceBetween: 15,
